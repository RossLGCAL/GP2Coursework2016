...
GameObject::GameObject()
{
	...
	m_Position1 = vec3(0.0f, 0.0f, 0.0f);
	m_Rotation1 = vec3(0.0f, 0.0f, 0.0f);
	m_Scale1 = vec3(1.0f, 1.0f, 1.0f);
...
void GameObject::onRender(mat4& view, mat4& projection)
{
	...
	glBindSampler(2, m_diffuseSampler);
...
void GameObject::onDestroy()
{
	...
	glDeleteSamplers(1, &m_diffuseSampler);
	glDeleteSamplers(1, &m_specularSampler);
...
void GameObject::loadDiffuseTexture(const string & filename)
{
	...
	glGenSamplers(1, &m_diffuseSampler);
	glSamplerParameteri(m_diffuseSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glSamplerParameteri(m_diffuseSampler, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glSamplerParameteri(m_diffuseSampler, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glSamplerParameteri(m_diffuseSampler, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glGenSamplers(1, &m_specularSampler);
	glSamplerParameteri(m_specularSampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glSamplerParameteri(m_specularSampler, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glSamplerParameteri(m_specularSampler, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glSamplerParameteri(m_specularSampler, GL_TEXTURE_WRAP_T, GL_REPEAT);
